...
class OxygenPredictor(KnowledgeEngine):
    def __init__(self, fish_type="standard"):
        super().__init__()
        ...
        self.fish_type = fish_type  # Store fish type
    ...
    def standard_rules(self):
        """Rules for standard fish type (original rules)."""
        if self.fish_type == "standard":
            # === Oxygen Rules ===
    ...
    def catfish_rules(self):
        """Rules specific to catfish."""
        if self.fish_type == "catfish":
            # pH rules for catfish
            ...
            @Rule(Fact(ph_level=MATCH.ph & P(lambda x: x > 8.0)))
            ...
            def low_temp_catfish(self, temp):
                self.add_issue(
                    "⚠️ Low temperature detected! Catfish prefer temperatures between 25°C and 32°C.",
                    "Increase water temperature gradually using a heater or by reducing water flow.",
                    severity=3,
                    category="temperature",
                    prediction="Fish may become lethargic and stop eating if temperatures remain low."
                )

            @Rule(Fact(temperature=MATCH.temp & P(lambda x: x > 32)))
            def high_temp_catfish(self, temp):
                self.add_issue(
            ...
            # Ammonia rules for catfish
            @Rule(Fact(ammonia=MATCH.amm & P(lambda x: x > 3)))
            ...
            # Dissolved oxygen rules for catfish
            @Rule(Fact(dissolved_oxygen=MATCH.do & P(lambda x: x < 4)))
            def low_oxygen_catfish(self, do):
                self.add_issue(
    ...
    def tilapia_rules(self):
        """Rules specific to tilapia."""
        if self.fish_type == "tilapia":
            # pH rules for tilapia
            @Rule(Fact(ph_level=MATCH.ph & P(lambda x: x < 6.5)))
            ...
            @Rule(Fact(temperature=MATCH.temp & P(lambda x: x < 26)))
            def low_temp_tilapia(self, temp):
                self.add_issue(
                    "⚠️ Low temperature detected! Tilapia prefer temperatures between 26°C and 30°C.",
                    "Increase water temperature gradually using a heater or by reducing water flow.",
                    severity=3,
                    category="temperature",
                    prediction="Fish may become lethargic and stop eating if temperatures remain low."
            ...
            def high_temp_tilapia(self, temp):
                self.add_issue(
            ...
            # Ammonia rules for tilapia
            @Rule(Fact(ammonia=MATCH.amm & P(lambda x: x > 2)))
            ...
            # Dissolved oxygen rules for tilapia
            @Rule(Fact(dissolved_oxygen=MATCH.do & P(lambda x: x < 5)))
            def low_oxygen_tilapia(self, do):
                self.add_issue(
    ...
    def crayfish_rules(self):
        """Rules specific to crayfish."""
        if self.fish_type == "crayfish":
            # pH rules for crayfish
            @Rule(Fact(ph_level=MATCH.ph & P(lambda x: x < 6.5)))
            ...
            def low_temp_crayfish(self, temp):
                self.add_issue(
                    "⚠️ Low temperature detected! Crayfish prefer temperatures between 18°C and 24°C.",
                    "Increase water temperature gradually using a heater or by reducing water flow.",
                    severity=3,
                    category="temperature",
                    prediction="Fish may become lethargic and stop eating if temperatures remain low."
            ...
            @Rule(Fact(temperature=MATCH.temp & P(lambda x: x > 24)))
            def high_temp_crayfish(self, temp):
                self.add_issue(
            ...
            # Ammonia rules for crayfish
            @Rule(Fact(ammonia=MATCH.amm & P(lambda x: x > 1)))
            ...
            # Dissolved oxygen rules for crayfish
            @Rule(Fact(dissolved_oxygen=MATCH.do & P(lambda x: x < 5)))
            def low_oxygen_crayfish(self, do):
                self.add_issue(
...
def predict():
    ...
    predictor = OxygenPredictor(fish_type=fish_type)
    predictor.reset()
